 ...
 */
public class QwSwichBtn extends View {
	/** 仿ios圆类型 */
	...
	public QwSwichBtn(Context context, AttributeSet attrs, int defStyle) {
		super(context, attrs, defStyle);
		init(context, attrs);
	}

	public QwSwichBtn(Context context, AttributeSet attrs) {
		super(context, attrs);
		init(context, attrs);
	}

	private void init(Context context, AttributeSet attrs) {
		TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.QwSwichBtn);
		style = a.getInteger(R.styleable.QwSwichBtn_btn_style, ROUND);
		bg_open = a.getColor(R.styleable.QwSwichBtn_bg_open, 0xff45C01A);
		bg_close = a.getColor(R.styleable.QwSwichBtn_bg_close, 0xffAAAAAA);
		bg_line = a.getColor(R.styleable.QwSwichBtn_bg_line, 0xffAAAAAA);
		btn_bg = a.getColor(R.styleable.QwSwichBtn_btn_bg, 0xffffffff);
		btn_line = a.getColor(R.styleable.QwSwichBtn_btn_line, 0xffffffff);
		mChecked = a.getBoolean(R.styleable.QwSwichBtn_checked, false);
		sw = dip2px(context, 1);
	...
	protected void onDraw(Canvas canvas) {
		...
		if (bg_rect == null && getWidth() != 0) {
			bg_rect = new RectF(0 + step, 0 + step, getWidth() - step, getHeight() - step);
			int width = 0;
			if (style == ROUND) {
				width = getHeight();
			} else {
			...
			btn_rect = new RectF(0 + sw + step, 0 + sw + step, width - sw - step, getHeight() - sw - step);
			...
			btn_rect2 = new RectF(beginLeft, 0 + sw + step, beginLeft + differ, getHeight() - sw - step);
	...
	public interface OnCheckedChangeListener {
		void onCheckedChanged(QwSwichBtn qwSwichBtn, boolean isChecked);
	}
